...
public partial class usercontrols_SecurityPropertyDetails : System.Web.UI.UserControl
{
    #region Properties
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            Property = REIQ.Access.Property.SecurityGetFromPropertyId(this.PropertyId);
        }
        catch (Exception ex)
        {
        ...
        if (Property != null)
        {
            ...
            if (featureCounter > 0)
            {
                if (landCounter > 0)
                {
                    ...
                    {
                        if (Property.lotSize > 0)
                            morefeaturesdisplay = morefeaturesdisplay + "<li>" + Property.lotSize + " " + Property.lotMeasurement + "</li>";
                    }
                    if (!String.IsNullOrEmpty(Property.lotFrontage))
                    {
