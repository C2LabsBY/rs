<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AgentPropertiesResult.ascx.cs" Inherits="usercontrols_AgentPropertiesResult" %>
...
<asp:Repeater runat="server" ID="rptProperties" OnItemDataBound="rptProperties_ItemDataBound">
    <ItemTemplate>
        <section class="search-result-block clearfix">
            <a href="<%# "/" + REIQ.Helpers.PropertyHelper.GenerateURL(((REIQ.Entities.Property)Container.DataItem).suburb.ToString(), ((REIQ.Entities.Property)Container.DataItem).pID.ToString(), ((REIQ.Entities.Property)Container.DataItem).type.ToString(), ((REIQ.Entities.Property)Container.DataItem).status.ToString(), ParamSearchType, ((REIQ.Access.PropertyExtended)Container.DataItem).region.ToString()) %>">
                <!-- SEARCH RESULT BLOCK -->
